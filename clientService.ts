    ...
    userRows.forEach(row => {
      ...
      subs.forEach(s => {
          if (s) {
              ...
              // Logic to populate subDetails with the best expiration date
              if (!subDetails[s]) {
                  subDetails[s] = { purchaseDate: row.purchase_date, durationMonths: row.duration_months };
              } else {
                  const currentStored = subDetails[s];
